    ...
    for row1 in f1_rows:
        for row2 in f2_rows:
            ...
            for v1, v2 in zip(row1, row2):
                # print(f'{v2} - {v1} = {v2 - v1}')
                row_result.append(abs(v2 - v1) <= 3 and abs(v2-v1) >=0)
            if all(row_result):
                result.append(row2)
    return result
...
if __name__ == '__main__':
    main(dir1=Path(r'/home/UNT/sd0570/mmdetection/Remove boxes equal to each other/lyamda55_one'),
         dir2=Path(r'/home/UNT/sd0570/mmdetection/Remove boxes equal to each other/lyamda55_two'))
