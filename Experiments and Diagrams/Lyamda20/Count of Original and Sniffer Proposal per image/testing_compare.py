    ...
    for row2 in f2_rows:
        for row1 in f1_rows:
            ...
            for v1, v2 in zip(row2, row1):
                # print(f'{v2} - {v1} = {v2 - v1}')
                row_result1.append(abs(v2 - v1) <= 20)
            if all(row_result1):
                result1.append(row1)
    return result1

def compare_content2(f1_rows):
    result = []
    for row1 in f1_rows:
...
def main(dir1, dir2):
    for file_one in dir1.glob('*.txt'):
        ...
        #result2 = compare_content2(f1_content1)
        print_result(result)
...
if __name__ == '__main__':
    main(dir1=Path(r'/home/UNT/sd0570/mmdetection/Experiments and Diagrams/Lyamda20/Count of Original and Sniffer Proposal per image/original'),
         dir2=Path(r'/home/UNT/sd0570/mmdetection/Experiments and Diagrams/Lyamda20/Count of Original and Sniffer Proposal per image/sniffer'))
