    ...
    for row1 in f1_rows:
        for row2 in f2_rows:
            ...
            for v1, v2 in zip(row1, row2):
                # print(f'{v2} - {v1} = {v2 - v1}')
                row_result.append(abs(v2 - v1) <= 145)
            if all(row_result):
                result.append(row2)
    return result
    ...
    for row2 in f2_rows:
        for row1 in f1_rows:
            ...
            for v1, v2 in zip(row2, row1):
                # print(f'{v2} - {v1} = {v2 - v1}')
                row_result1.append(abs(v2 - v1) <= 145)
            if all(row_result1):
                result1.append(row1)
    return result1
...
if __name__ == '__main__':
    main(dir1=Path(r'/home/UNT/sd0570/mmdetection/Lyamda Values/original'),
         dir2=Path(r'/home/UNT/sd0570/mmdetection/Lyamda Values/sniffer'))
