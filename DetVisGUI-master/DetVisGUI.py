...
parser.add_argument('--gt_box_color', default=(255, 0, 0), help='groundtruth box color')
...
class COCO_dataset:
    ...
    def json_parser(self, train_anno, has_anno):
        ...
        if has_anno:
            for a in annotations:
                image_name = image_dict[a["image_id"]][0].replace('.png', '')
    ...
    def get_singleImg_gt(self, name):
        if name.replace('.png', '') not in self.total_annotations.keys():
            print('There are no annotations in %s.' % name)
            return []
        else:
            return self.total_annotations[name.replace('.png', '')]
...
class VOC_dataset:
    def __init__(self):
        ...
        self.results = self.get_det_results() if self.det_file != '' else None
        self.aug_category = aug_category(['car','person'])
    ...
    def get_img_list(self):
        ...
        return [x.strip() + ".png" for x in data]
    ...
    def get_singleImg_gt(self, name):       # get annotations by image name
        ...
        xml_path = os.path.join(self.data_root, 'Annotations', name.replace('.png','') + ".xml")
        ...
        for obj in root.findall('object'):
            ...
            bbox = [
                ...
                int(float(bndbox.find('xmax').text)) - int(float(bndbox.find('xmin').text)),
                int(float(bndbox.find('ymax').text)) - int(float(bndbox.find('ymin').text))
            ]
